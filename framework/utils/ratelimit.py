...
class RateLimitManager:
    ...
    def update(self, _type: str, _id: str, rate: int):
        ...
        q = Query()
        self.limit_db.upsert({"type": _type, "id": _id, "rate": rate}, q.fragment(
            {"type": _type, "id": _id}))
    ...
    def update_draw(self, _type: str, _id: str, rate: int):
        ...
        q = Query()
        self.draw_limit_db.upsert(
            {"type": _type, "id": _id, "rate": rate}, q.fragment({"type": _type, "id": _id}))
    ...
    def get_draw_limit(self, _type: str, _id: str) -> Document:
        ...
        if entity is None and _id != '默认':
            return self.draw_limit_db.get(
                q.fragment({"type": _type, "id": '默认'}))
        return entity
    ...
    def get_usage(self, _type: str, _id: str) -> Document:
        ...
        # 删除过期的记录
        time_diff_dondition = (
            usage is not None and usage.get('time') != current_time)
        day_diff_condition = (
            usage is not None and usage.get('time') == current_time and usage.get('day') != current_day)
    ...
    def increment_usage(self, _type, _id):
        ...
        q = Query()
        self.usage_db.update(increment('count'),
                             q.fragment({"type": _type, "id": _id}))
    ...
    def increment_draw_usage(self, _type, _id):
        ...
        q = Query()
        self.draw_usage_db.update(
            increment('count'), q.fragment({"type": _type, "id": _id}))
